...
void graph::gen_vertices(int n)
{
	int i=0;
	for(i=0;i<n;i++)
	{
		p[i]=0;
		vertices.pb(mp(i+1,INF));
	}
}
...
void graph::MST_Prim()
{
	...
	while(ni.heapSize()!=0)
	{
		...
		if(p[temp.first-1]!=0)
		{
			ans.second.pb(mp(temp.second,mp(p[temp.first-1],temp.first)));
			cost+=temp.second;
		}
		for(itr=adj[temp.first-1].begin();itr!=adj[temp.first-1].end();itr++)
		{
			v=*itr;
			if(ni.inHeap(v.first)&&v.second<vertices[v.first-1].second)
			{
				p[v.first-1]=temp.first;
				vertices[v.first-1].second=v.second;
...
void graph::printGraph()
{
	...
	for(i=0;i<vertices.size();i++)
		cout<<vertices[i].first<<" key= "<<vertices[i].second<<endl;
	cout<<endl;
	cout<<"Edges are:\n";
